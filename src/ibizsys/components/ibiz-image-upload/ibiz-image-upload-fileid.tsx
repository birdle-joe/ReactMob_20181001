...
export class IBizImageUpload extends Component<IBizImageUploadProps> {
    ...
    public setfiles(fileid: any): [] {
        let $files: any = [];
        if (fileid) {
            let filename = "image.png";
            let paramURL = IBizEnvironment.ExportFile + `?fileid=${fileid}`;
            paramURL = (IBizEnvironment.LocalDeve ? '' : '..') + paramURL;
            let f={id:fileid,name:filename,url:paramURL}
            $files.push(f);
        }
        return $files;
    }
